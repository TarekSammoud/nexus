...
export class ChatRoomService {
  ...
  connect(roomId: string, userId: number, token: string): void {
    ...
    this.stompClient = new Client({
      ...
      reconnectDelay: 5000,
      webSocketFactory: () => new SockJS('http://localhost:9000/nexus-backend/ws-support'),
      connectHeaders: {
  ...
  getMessages(roomId: string): Observable<ChatMessage[]> {
    return this.http.get<ChatMessage[]>(`http://localhost:9000/nexus-backend/msg/getRoomMessages?roomId=${roomId}`);
  }

  getRoomUsers(roomId: string): Observable<User[]> {
    return this.http.get<User[]>(`http://localhost:9000/nexus-backend/chat-room/users?roomId=${roomId}`);
  }
