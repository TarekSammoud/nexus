...
export class WebsocketSupportService {
  ...
  connect(token: string, roomId:Number, onMessage: (msg: any) => void): void {
    ...
    const socket = new WebSocket('ws://localhost:9000/nexus-backend/ws-support');
  ...
  private apiUrl = '  http://localhost:9000/nexus-backend/user';  // Replace with your actual Spring Boot server URL
