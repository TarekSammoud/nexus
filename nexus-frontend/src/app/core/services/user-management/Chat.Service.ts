...
export class ChatService {
    ...
    connect(userId: number): void {
        ...
        this.stompClient = new Client({
            brokerURL: '/ws_api/nexus-backend/ws',
            reconnectDelay: 5000,
            webSocketFactory: () => new SockJS('/api/nexus-backend/ws'),
            onConnect: () => {
    ...
    getMessages(userId: number): Observable<ChatMessage[]> {
        return this.http.get<ChatMessage[]>(`/api/nexus-backend/msg/getMessages?userId=${userId}`);
    }
}